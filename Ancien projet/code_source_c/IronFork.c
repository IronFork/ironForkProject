...
int main()
{
  int longueurFichier=400;
  char* nom_fichier= "Configuration.csv";
  double trajModel[longueurFichier][6];
  ...
  while (fgets( ligne, nbLignes, fichier))
  {
  	...
  	while (ptr_chain != NULL)
  	{
  		...
  		trajModel[num_ligne-1][num_colonne]=data;
  ...
  */
  ...
  while (1)
  {
    ...
    // Write goal position
    dxl1_goal_position = trajModel[i][1];
    dxl2_goal_position = trajModel[i][2];
    dxl3_goal_position = trajModel[i][3];
    dxl4_goal_position = trajModel[i][4];
    dxl5_goal_position = trajModel[i][5];
    ...
    usleep((trajModel[i+1][0]-trajModel[i][0])*1e6);
    
    /*if (i==longueurFichier/2)
    {
    	getch();
    }
    */
